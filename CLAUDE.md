...
- **Image reuse:** ~14 source photos for 20+ slots, so several repeat — straw-hat `5E6A9822` (parties hero + camps tile + Two Brothers), old-man `5E6A9886` (About + vent card + Garbage show), guitar `5E6A9943` (home hero + parties tile), `Cow 2` (puppet card + Cow show), `count-king2` (schools tile; **no longer** the Monkey show — Monkey now has its own `IMG_8221` Poritz+monkey photo), `5E6A0005` (Unhappy King, home+shows). Owner to send more photos to de-dupe (list in `TODO.md`).
