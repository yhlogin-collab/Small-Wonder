...
- **About** — BUILT (`about.html`): "Puppetry with a purpose" mission + credentials up top (since 1978, hundreds of thousands of kids, ~5,000 shows, CBS Chanukah special, Jewish Children's Museum, across the spectrum of observance + secular schools), then the origin **story** lower down (Ufaratzta Puppet Theater → a shiur at Bais Rivka → an overnight show → Buffalo → "teach me"; handmade community puppets; the copper travelling stage; Rav-reviewed scripts; the "make someone else happy" mission via *The Unhappy King*; Uncle Pinchy → ventriloquism). Names **Rabbi Yaakov Horowitz** (naming is now OK). Photos `about-1/2/3`.
- **FAQ** — BUILT (`faq.html`): practical Q&A (show format ~45 min w/ 2–3 puppet shows + musical program, ages 3–10, audiences 20–500, self-contained sound/lighting, travel area = **Tri-State NY/NJ/CT + Dallas** + seasonal elsewhere, space, puppet-vs-ventriloquist, Rav-reviewed content, lead time). Linked in the main nav + footers.
