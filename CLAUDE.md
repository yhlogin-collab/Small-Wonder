...
**Logo & motif:** the Wonder Entertainment logo (royal purple wordmark, gold outline, blue "ENTERTAINMENT", arc of multi-colored **six-pointed Magen David stars**). The site adopts that motif: the hero decorations and the nav brand mark are six-pointed stars (CSS `--star` mask) in the logo colors, and the palette gained `--purple #6E2C9D` / `--purple-deep` / `--green #6FB63C`. Small Wonder Puppet Theater stays the lead brand (nav wordmark); the actual Wonder Entertainment logo image belongs in the footer (pending: owner to attach a transparent PNG — inline pastes don't persist to disk).
