...
- **Our Shows** — BUILT (`shows.html`): the two formats (Puppet Show + Ventriloquist) + a "most popular shows" section of **5 real shows** with owner-written descriptions (The Cow That Kept Shabbat · The Two Brothers · The Unhappy King · The Garbage and the Kotel · The Monkey with the Gold Coins). Each `.show-feature .videocard` is **poster-only until per-show YouTube IDs arrive** — add `data-yt="<id>"` to a card to turn on click-to-play. Posters `show-1..5`; **show-5 (Monkey) is now a real Poritz + monkey stage photo** (from `IMG_8221`, EXIF-rotated upright, 16:9 crop), and **show-3 (Garbage and the Kotel) is now a real Kotel-puppet stage photo** (from `IMG_0881`, 16:9 crop) — no longer the reused old-man photo. The homepage keeps a 2-format teaser (`#shows`); the standalone `cast-*` images are no longer used on any page (owner dropped the cast gallery) but remain available for reuse.
...
- **Image reuse:** ~14 source photos for 20+ slots, so several repeat — straw-hat `5E6A9822` (parties hero + camps tile + Two Brothers), old-man `5E6A9886` (About `about-1` + vent card; **no longer** the Garbage show — that's now the real Kotel photo), guitar `5E6A9943` (home hero + parties tile), `Cow 2` (puppet card + Cow show), `count-king2` (schools tile; **no longer** the Monkey show — Monkey now has its own `IMG_8221` Poritz+monkey photo), `5E6A0005` (Unhappy King, home+shows). Owner to send more photos to de-dupe (list in `TODO.md`).
