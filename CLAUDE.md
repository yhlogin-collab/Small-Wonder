...
- **About** — BUILT (`about.html`): "Puppetry with a purpose" mission + credentials up top (since 1978, hundreds of thousands of kids, ~5,000 shows, CBS Chanukah special, Jewish Children's Museum, Javits Center Expo), then the origin **story** lower down (Ufaratzta Puppet Theater → a shiur at Bais Rivka → an overnight show → Buffalo → "teach me"; handmade community puppets; the copper travelling stage; the "make someone else happy" mission via *The Unhappy King*; Uncle Pinchy → ventriloquism). Names **Rabbi Yaakov Horowitz** (naming is now OK). Photos `about-1/2/3`. (Kosher/Rav-review section removed per owner.)
